let
  ...
  in
    ...
    then throw "Could not find project root with flake.nix"
  ...
  loadEnvVars = let
    ...
    envFile =
      if pwdExists
      then envFileInPwd
      else envFileInProject;
  in
    if isFlakeCheck
    then defaultEnv
    else if pwdExists || projectExists
    then let
      envContent = builtins.readFile envFile;
      parseEnvFile = content: let
        trim = s: let
          m = builtins.match "^[[:space:]]*(.*[^[:space:]])?[[:space:]]*$" s;
        in
          if m == null
          then s
          else if (builtins.elemAt m 0) == null
          then ""
    ...
    else throw "Required .env file not found at ${envFileInPwd} or ${envFileInProject}. Please create it with USERNAME, NAME, EMAIL, HOSTNAME variables.";
  ...
  in
    if missing != []
    then throw "Missing required environment variables in .env: ${builtins.concatStringsSep ", " missing}"
    else env;

...
in
{
  options.igloo.tmux.enable = lib.mkEnableOption "Enable tmux configuration" // {
    default = true;
  };
  options.igloo.tmux.pickers.enable = lib.mkEnableOption "Enable fzf-based tmux pickers (F1–F4)" // {
  ...
  config = lib.mkIf config.igloo.tmux.enable {
    programs.tmux = {
      ...
      extraConfig =
        let
          pickerBinds =
            if config.igloo.tmux.pickers.enable then
              ''
                # Custom fzf pickers
